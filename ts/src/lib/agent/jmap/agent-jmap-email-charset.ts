...
function isTextStarType(type: string): boolean {
  return baseMediaType(type).startsWith("text/");
}

/** RFC 8621 EmailBodyPart: never add charset when partId is set. */
function ensureCharsetOnBodyPart(part: unknown): void {
  if (!part || typeof part !== "object" || Array.isArray(part)) return;
  const o = part as Record<string, unknown>;
  if (typeof o.partId === "string" && o.partId.length > 0) return;
  if (typeof o.blobId !== "string" || o.blobId.length === 0) return;
  const t = o.type;
  if (typeof t !== "string" || !isTextStarType(t)) return;
  if (o.charset != null && o.charset !== "") return;
  o.charset = DEFAULT_TEXT_CHARSET;
}

function normalizeBodyPartArray(arr: unknown): void {
  if (!Array.isArray(arr)) return;
  for (const item of arr) ensureCharsetOnBodyPart(item);
}

function normalizeEmailSetArg(arg: unknown): void {
  if (!arg || typeof arg !== "object" || Array.isArray(arg)) return;
  const create = (arg as Record<string, unknown>)["create"];
  if (!create || typeof create !== "object" || Array.isArray(create)) return;
  for (const email of Object.values(create as Record<string, unknown>)) {
    if (!email || typeof email !== "object" || Array.isArray(email)) continue;
