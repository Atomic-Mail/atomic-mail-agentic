...
export function decodedBase64ByteLength(b64: string): number {
  const t = b64.replace(/\s+/g, "");
  if (t.length === 0) return 0;
  if (t.length % 4 === 1) {
    throw new Error(
      "Invalid base64 in Blob/upload data:asBase64 (RFC 9404 §4.1): bad length.",
    );
  }
  try {
    ...
    throw new Error(
      "Invalid base64 in Blob/upload data:asBase64 (RFC 9404 §4.1).",
    );
  }
}

function sliceOctetCount(
  sourceLen: number,
  offset: unknown,
  length: unknown,
): number {
 ...
 */
export function tryComputeUploadDataOctets(
  dataUnknown: unknown,
  knownSizes: ReadonlyMap<string, number>,
): number | null {
  if (!Array.isArray(dataUnknown)) return 0;
  ...
  for (const part of dataUnknown) {
    ...
    if (n !== 1) {
      throw new Error(
        "Each Blob/upload DataSourceObject must use exactly one of " +
...
function resolveCreateSizesForOneBlobUpload(
  create: Record<string, Record<string, unknown>>,
  limits: JmapBlobUploadLimits,
  priorSizes: ReadonlyMap<string, number>,
): Map<string, number> {
  const merged = new Map<string, number>(priorSizes);
  ...
  while (pending.size > 0) {
    ...
    for (const id of [...pending]) {
      ...
      if (
        limits.maxDataSources !== undefined &&
        dsCount > limits.maxDataSources
      ) {
        throw new Error(
          `Blob/upload create "${id}" uses ${dsCount} DataSourceObject entries; ` +
      ...
      if (
        limits.maxSizeBlobSet !== null &&
        computed > limits.maxSizeBlobSet
      ) {
        throw new Error(
          `Blob/upload create "${id}" would be ${computed} octets; account ` +
 ...
 */
export function assertBlobUploadEnvelopeWithinLimits(
  envelope: JmapEnvelopeLike,
  limitsByAccount: ReadonlyMap<string, JmapBlobUploadLimits | null>,
): void {
  let globalSizes = new Map<string, number>();
  ...
  for (const call of envelope.methodCalls) {
    ...
    globalSizes = resolveCreateSizesForOneBlobUpload(
...
export function assertAttachmentBytesWithinBlobLimit(
  items: readonly { label: string; byteLength: number }[],
  limits: JmapBlobUploadLimits | null,
): void {
  if (!limits || limits.maxSizeBlobSet === null) return;
  const max = limits.maxSizeBlobSet;
  for (const it of items) {
    if (it.byteLength > max) {
      throw new Error(
        `${it.label} is ${it.byteLength} octets but account maxSizeBlobSet is ` +
