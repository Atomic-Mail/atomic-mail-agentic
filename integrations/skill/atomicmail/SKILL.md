...
- **Credentials directory:** Default `~/.hermes/atomicmail` (not `~/.atomicmail`). The skill launcher sets `ATOMIC_MAIL_CREDENTIALS_DIR` when unset; operator env or `atomicmail.credentials_dir` config overrides it.
- **After register:** Accept the hourly inbox blueprint via `/suggestions` — do not skip inbox polling setup.
- **Never cron raw CLI:** Do not schedule `${HERMES_SKILL_DIR}/scripts/atomicmail jmap_request` alone without an agent turn. The blueprint uses `no_agent: false` so each run is a full agent session with `list_inbox.json`.
